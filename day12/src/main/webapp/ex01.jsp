...
<h3>문자열로 형태를 저장해두면, 다음번 방문 시 서버가 값을 회수하여 활용할 수 있다</h3>
<h3>쿠키는 일정시간이 지나면 자동으로 소멸하게 되어있다</h3>
...
<fieldset>
	...
	<%
		...
		//	첫번째 방문이라면 cookies 배열의 값은 null 일 수도 있다
		Cookie[] cookies = request.getCookies();
	
	
		//	쿠키의 값은 항상 문자열 형태. 쿠키의 값을 저장할 변수를 미리 만들어둔다
		String value = "";
		
		//	cookies가 null이면 cookies.length 에서 NullPointerException 이 발생할 수 있다
		for(int i = 0; cookies != null && i < cookies.length; i++) {
			Cookie c = cookies[i];
			if(c.getName().equals("memo")) {	//	내가 저장한 이름과 일치하는 쿠키를 찾는다 
				value = c.getValue();
				break;
			}
