	...
	<tr>
		...
		<td>
			<ul>
				...
				<li>request, response 는 새로 만들어진다</li>
			</ul>
		</td>
	<tr>
		<td>response.getRequestDispather(String uri).forward(request, response)</td>
		<td>
			<ul>
				<li>서버 내부에서 임의로 응답할 주소를 고르기 때문에, 다른 서버로는 넘길 수 없다</li>
				<li>클라이언트는 관여하지 않고, 서버에서 응답을 바꿔치기하므로 브라우저 주소창이 변경되지 않는다</li>
				<li>고객에게 잠시 기다리게 하고, 내선번호로 전화를 돌리는 형식</li>
			</ul>
		</td>
	</tr>
...
<fieldset>
	...
	<h3>request 와 response 를 이용하여 페이지 이동 </h3>
	<h3>리퀘, 리폰 안쓸거면 그냥 편하게 리다이렉트 써라</h3>
	<h3>또한 페이지 주소를 보여주고 싶지 않을때에도 사용한다</h3>
	<form>
